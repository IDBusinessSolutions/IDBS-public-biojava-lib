 ...
 */
/**
 * This class is a clone of EmblFormat with the following modifications:
 * 
...
import org.biojavax.bio.taxa.*;
import org.biojavax.ontology.*;
import org.biojavax.utils.*;


/**
 * This Class extends the {@link EMBLFormat} to provide specific functionality.
 *
 ...
 * @since 1.5
 */
public class BjEmblFormat extends RichSequenceFormat.HeaderlessFormat
{

    // Register this format with the format auto-guesser.
    static
    {
        BjRichSequence.IOTools.registerFormat(BjEmblFormat.class);
    }
    ...
    // locus line
    protected static final Pattern lp = Pattern
            .compile("^(\\S+);\\s+SV\\s+(\\d+);\\s+(linear|circular);\\s+(\\S+\\s?\\S+?);\\s+(\\S+);\\s+(\\S+);\\s+(\\d+)\\s+BP\\.$");

    protected static final Pattern lpPre87 = Pattern
            .compile("^(\\S+)\\s+standard;\\s+(circular)?\\s*(genomic)?\\s*(\\S+);\\s+(\\S+);\\s+\\d+\\s+BP\\.$");
    ...
    /**
     * {@inheritDoc} A file is in EMBL format if its name contains the word eem or edat, or the first line matches the
     * EMBL format for the ID line.
     */
    public boolean canRead(File file) throws IOException
    {
        ...
        boolean readable = firstLine != null
            && headerLine.matcher(firstLine).matches()
            && (lp.matcher(firstLine.substring(3).trim()).matches() || lpPre87.matcher(firstLine.substring(3).trim())
                    .matches());
        br.close();
        return readable;
    }
    ...
    /**
     * {@inheritDoc} A stream is in EMBL format if its first line matches the EMBL format for the ID line.
     */
     ...
     * (non-Javadoc)
     * 
     * @see org.biojavax.bio.seq.io.EMBLFormat#guessSymbolTokenization(java.io.BufferedInputStream)
     */
    @Override
    public SymbolTokenization guessSymbolTokenization(final BufferedInputStream stream) throws IOException
    {

        return RichSequence.IOTools.getNucleotideParser();
    }
     ...
     * {@inheritDoc}
     */
    public boolean readRichSequence(BufferedReader reader, SymbolTokenization symParser, RichSeqIOListener rlistener,
            Namespace ns) throws IllegalSymbolException, IOException, ParseException
    {
        ...
        do
        {
            ...
            if (sectionKey == null)
            {
                ...
                String message = ParseException.newMessage(this.getClass(), accession, "No section key", "Not set",
            ...
            if (sectionKey.equals(LOCUS_TAG))
            {
                ...
                else
                {
                    String message = ParseException.newMessage(this.getClass(), accession, "Not Set",
                        "Bad ID line found", sectionToString(section));
                    throw new ParseException(message);
                }
            }
            ...
            else if (sectionKey.equals(DATE_TAG))
            {
                ...
                if (dm.matches())
                {
                    ...
                    {
                        String message = ParseException.newMessage(this.getClass(), accession, "not set",
                            "Bad date type found", sectionToString(section));
                        throw new ParseException(message);
                    }
                }
                else
                {
                    String message = ParseException.newMessage(this.getClass(), accession, "not set",
                        "Bad date line found", sectionToString(section));
                    throw new ParseException(message);
            ...
            else if (sectionKey.equals(DATABASE_XREF_TAG))
            {
                ...
                CrossRef crossRef = (CrossRef)RichObjectFactory.getObject(SimpleCrossRef.class, new Object[]
                { parts[0].trim(), parts[1].trim(), new Integer(0) });
                ...
                for (int j = 2; j < parts.length; j++)
                {
                    ...
                    {
                        String message = ParseException.newMessage(this.getClass(), accession, "not set",
                            "Could not annotate identifier terms", sectionToString(section));
            ...
            else if (sectionKey.equals(REFERENCE_TAG) && !this.getElideReferences())
            {
                ...
                for (int i = 1; i < section.size(); i++)
                {
                    ...
                    if (key.equals(REFERENCE_POSITION_TAG))
                    {
                        ...
                        {
                            String message = ParseException.newMessage(this.getClass(), accession, "not set",
                                "Bad reference line found", sectionToString(section));
                            throw new ParseException(message);
                        }
                    }
                }
                ...
                try
                {
                    ...
                    DocRef dr = (DocRef)RichObjectFactory.getObject(SimpleDocRef.class, new Object[]
                    { authSet, locator, title });
                    // assign either the pubmed or medline to the docref - medline gets priority, then pubmed, then doi
                    if (medline != null)
                        dr.setCrossref((CrossRef)RichObjectFactory.getObject(SimpleCrossRef.class, new Object[]
                        { Terms.MEDLINE_KEY, medline, new Integer(0) }));
                    else if (pubmed != null)
                        dr.setCrossref((CrossRef)RichObjectFactory.getObject(SimpleCrossRef.class, new Object[]
                        { Terms.PUBMED_KEY, pubmed, new Integer(0) }));
                    else if (doi != null)
                        dr.setCrossref((CrossRef)RichObjectFactory.getObject(SimpleCrossRef.class, new Object[]
                        { Terms.DOI_KEY, doi, new Integer(0) }));
                    // assign the remarks
                    if (!this.getElideComments())
                        dr.setRemark(remark);
                    // assign the docref to the bioentry
                    RankedDocRef rdr = new SimpleRankedDocRef(dr, (ref_start != -999 ? new Integer(ref_start) : null),
                        (ref_end != -999 ? new Integer(ref_end) : null), ref_rank);
                ...
                for (int i = 1; i < section.size(); i++)
                {
                    ...
                    if (key.startsWith("/"))
                    {
                        ...
                        else
                        {
                            ...
                            rlistener.addFeatureProperty(RichObjectFactory.getDefaultOntology().getOrCreateTerm(key),
                                val);
                        }
                    }
                    else
            ...
            else if (sectionKey.equals(START_SEQUENCE_TAG) && !this.getElideSymbols())
            {
                ...
                try
                {
                    SymbolList sl = new SimpleSymbolList(symParser, seq.toString().replaceAll("\\s+", "").replaceAll(
                        "[\\.|~]", "-"));
                    rlistener.addSymbols(symParser.getAlphabet(), (Symbol[])(sl.toList().toArray(new Symbol[0])), 0, sl
                            .length());
                }
                catch (Exception e)
                {
                    String message = ParseException.newMessage(this.getClass(), accession, "not set", "Bad sequence",
                        sectionToString(section));
                    throw new ParseException(e, message);
                }
            }
    ...
    private List readSection(BufferedReader br) throws ParseException
    {
        ...
        try
        {
            while (!done)
            {
                ...
                if (line.length() < 2)
                {
                    String message = ParseException.newMessage(this.getClass(), accession, "not set", "Bad line found",
                        line);
                    throw new ParseException(message);
                }
                ...
                if (token.equals(START_SEQUENCE_TAG))
                {
                    ...
                    section.add(new String[]
                    { START_SEQUENCE_TAG, sb.toString() });
                }
                ...
                else if (token.equals(FEATURE_HEADER_TAG))
                {
                    ...
                    while (!done)
                    {
                        ...
                        if (line.startsWith(DELIMITER_TAG))
                        {
                            ...
                            if (currentTag != null)
                                section.add(new String[]
                                { currentTag, currentVal.toString() });
                        }
                        else
                        {
                ...
                else if (token.equals(TPA_TAG))
                {
                    ...
                    String message = ParseException.newMessage(this.getClass(), accession, "not set",
                        "Unable to handle TPAs just yet", sectionToString(section));
                    throw new ParseException(message);
                }
                // READ CONTIG SECTION
                else if (token.equals(CONTIG_TAG))
                {
                    ...
                    String message = ParseException.newMessage(this.getClass(), accession, "not set",
                        "Unable to handle contig assemblies just yet", sectionToString(section));
                    throw new ParseException(message);
                }
                // READ DOCREF
                ...
                {
                    section.add(new String[]
                    { DATABASE_XREF_TAG, line.substring(5).trim() });
                    done = true;
                }
                // READ DATE
                ...
                {
                    section.add(new String[]
                    { DATE_TAG, line.substring(5).trim() });
                    done = true;
                }
    ...
    /**
     * As per {@link #writeSequence(Sequence, Namespace)}, except that it also takes a format parameter. This can be any
     * of the formats defined as constants in this class.
     * 
     * @param seq
     *            see {@link #writeSequence(Sequence, Namespace)}
     * @param format
     *            the format to use.
     * @param ns
     *            see {@link #writeSequence(Sequence, Namespace)}
    ...
    public void writeSequence(Sequence seq, String format, Namespace ns) throws IOException
    {
        ...
        StringTools.writeKeyValueLine(LOCUS_TAG, locusLine.toString(), 5, this.getLineWidth(), null, LOCUS_TAG, this
                .getPrintStream());
        this.getPrintStream().println(DELIMITER_TAG + "   ");

        // accession line
        StringTools.writeKeyValueLine(ACCESSION_TAG, accessions.toString(), 5, this.getLineWidth(), null,
            ACCESSION_TAG, this.getPrintStream());
        this.getPrintStream().println(DELIMITER_TAG + "   ");
        ...
        {
            if (versionLine != null)
                StringTools.writeKeyValueLine(VERSION_TAG, versionLine, 5, this.getLineWidth(), null, VERSION_TAG, this
                        .getPrintStream());
            else
                StringTools.writeKeyValueLine(VERSION_TAG, accession + "." + rs.getVersion(), 5, this.getLineWidth(),
                    null, VERSION_TAG, this.getPrintStream());
            this.getPrintStream().println(DELIMITER_TAG + "   ");
        }

        // date line
        StringTools.writeKeyValueLine(DATE_TAG, (cdat == null ? udat : cdat) + " (Rel. " + (crel == null ? "0" : crel)
            + ", Created)", 5, this.getLineWidth(), null, DATE_TAG, this.getPrintStream());
        StringTools.writeKeyValueLine(DATE_TAG, udat + " (Rel. " + (urel == null ? "0" : urel)
            + ", Last updated, Version " + (urecv == null ? "0" : urecv) + ")", 5, this.getLineWidth(), null, DATE_TAG,
            this.getPrintStream());
        this.getPrintStream().println(DELIMITER_TAG + "   ");

        // definition line
        StringTools.writeKeyValueLine(DEFINITION_TAG, rs.getDescription(), 5, this.getLineWidth(), null,
            DEFINITION_TAG, this.getPrintStream());
        this.getPrintStream().println(DELIMITER_TAG + "   ");
        ...
        {
            keywords.append(".");
            StringTools.writeKeyValueLine(KEYWORDS_TAG, keywords.toString(), 5, this.getLineWidth(), null,
                KEYWORDS_TAG, this.getPrintStream());
            this.getPrintStream().println(DELIMITER_TAG + "   ");
        }
        else
        {
        ...
        if (tax != null)
        {
            StringTools.writeKeyValueLine(SOURCE_TAG, tax.getDisplayName(), 5, this.getLineWidth(), null, SOURCE_TAG,
                this.getPrintStream());
            StringTools.writeKeyValueLine(ORGANISM_TAG, tax.getNameHierarchy(), 5, this.getLineWidth(), null,
                SOURCE_TAG, this.getPrintStream());
            if (organelle != null)
                StringTools.writeKeyValueLine(ORGANELLE_TAG, organelle, 5, this.getLineWidth(), null, ORGANELLE_TAG,
                    this.getPrintStream());
            this.getPrintStream().println(DELIMITER_TAG + "   ");
        }
        ...
        for (Iterator r = rs.getRankedDocRefs().iterator(); r.hasNext();)
        {
            ...
            StringTools.writeKeyValueLine(REFERENCE_TAG, "[" + rdr.getRank() + "]", 5, this.getLineWidth(), null,
                REFERENCE_TAG, this.getPrintStream());
            StringTools.writeKeyValueLine(REMARK_TAG, d.getRemark(), 5, this.getLineWidth(), null, REMARK_TAG, this
                    .getPrintStream());
            ...
            StringTools.writeKeyValueLine(REFERENCE_POSITION_TAG, rstart + "-" + rend, 5, this.getLineWidth(), null,
                REFERENCE_POSITION_TAG, this.getPrintStream());
            CrossRef c = d.getCrossref();
            if (c != null)
                StringTools.writeKeyValueLine(REFERENCE_XREF_TAG, c.getDbname() + "; " + c.getAccession() + ".", 5,
                    this.getLineWidth(), null, REFERENCE_XREF_TAG, this.getPrintStream());
            ...
            for (Iterator j = auths.iterator(); j.hasNext();)
            {
                ...
                if (a.isConsortium())
                {
                    StringTools.writeKeyValueLine(CONSORTIUM_TAG, a + ";", 5, this.getLineWidth(), null,
                        CONSORTIUM_TAG, this.getPrintStream());
                    j.remove();
                }
            }
            if (!auths.isEmpty())
                StringTools.writeKeyValueLine(AUTHORS_TAG, DocRefAuthor.Tools.generateAuthorString(auths, true) + ";",
                    5, this.getLineWidth(), null, AUTHORS_TAG, this.getPrintStream());
            else
                StringTools.writeKeyValueLine(AUTHORS_TAG, ";", 5, this.getLineWidth(), null, AUTHORS_TAG, this
                        .getPrintStream());
            if (d.getTitle() != null && d.getTitle().length() != 0)
                StringTools.writeKeyValueLine(TITLE_TAG, "\"" + d.getTitle() + "\";", 5, this.getLineWidth(), null,
                    TITLE_TAG, this.getPrintStream());
            else
                StringTools.writeKeyValueLine(TITLE_TAG, ";", 5, this.getLineWidth(), null, TITLE_TAG, this
                        .getPrintStream());
            StringTools.writeKeyValueLine(LOCATOR_TAG, d.getLocation() + ".", 5, this.getLineWidth(), null,
                LOCATOR_TAG, this.getPrintStream());
            this.getPrintStream().println(DELIMITER_TAG + "   ");
        }
        ...
        for (Iterator r = rs.getRankedCrossRefs().iterator(); r.hasNext();)
        {
            ...
            if (!hasSecondary)
                ...
                sb.append(".");
            StringTools.writeKeyValueLine(DATABASE_XREF_TAG, sb.toString(), 5, this.getLineWidth(), null,
                DATABASE_XREF_TAG, this.getPrintStream());
        }
        ...
        if (!rs.getComments().isEmpty())
        {
            ...
            StringTools.writeKeyValueLine(COMMENT_TAG, sb.toString(), 5, this.getLineWidth(), null, COMMENT_TAG, this
                    .getPrintStream());
            this.getPrintStream().println(DELIMITER_TAG + "   ");
        }
        ...
        for (Iterator i = rs.getFeatureSet().iterator(); i.hasNext();)
        {
            ...
            StringTools.writeKeyValueLine(FEATURE_TAG + "   " + f.getTypeTerm().getName(), GenbankLocationParser
                    .writeLocation((RichLocation)f.getLocation()), 21, this.getLineWidth(), ",", FEATURE_TAG, this
                    .getPrintStream());
            for (Iterator j = f.getNoteSet().iterator(); j.hasNext();)
            {
                ...
                if (n.getValue() == null || n.getValue().length() == 0)
                    StringTools.writeKeyValueLine(FEATURE_TAG, "/" + n.getTerm().getName(), 21, this.getLineWidth(),
                        null, FEATURE_TAG, this.getPrintStream());
                else
                    StringTools.writeKeyValueLine(FEATURE_TAG, "/" + n.getTerm().getName() + "=\"" + n.getValue()
                        + "\"", 21, this.getLineWidth(), null, FEATURE_TAG, this.getPrintStream());
            }
            ...
            if (f.getType().equals("source") && tax != null)
            {
                ...
                StringTools.writeKeyValueLine(FEATURE_TAG, "/organism=\"" + displayName + "\"", 21,
                    this.getLineWidth(), null, FEATURE_TAG, this.getPrintStream());
                StringTools.writeKeyValueLine(FEATURE_TAG, "/db_xref=\"taxon:" + tax.getNCBITaxID() + "\"", 21, this
                        .getLineWidth(), null, FEATURE_TAG, this.getPrintStream());
            }
            ...
            for (Iterator j = f.getRankedCrossRefs().iterator(); j.hasNext();)
            {
                ...
                StringTools.writeKeyValueLine(FEATURE_TAG, "/db_xref=\"" + cr.getDbname() + ":" + cr.getAccession()
                    + "\"", 21, this.getLineWidth(), null, FEATURE_TAG, this.getPrintStream());
            }
        }
        this.getPrintStream().println(DELIMITER_TAG + "   ");
