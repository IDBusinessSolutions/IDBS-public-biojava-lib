 ...
 */
/**
 * This class is a clone of GenbankFormat with the following modifications:
 *
...
/**
 * In order to handle file formats correctly the class {@link GenbankFormat} needed to be modified to provide the
 * correct typing of 'Nucleotide' and Protein. These changes were achieved by extending the class and over-riding the
 * guessSymbolTokenization(BufferedInputStream) method.
 *
...
public class BjGenbankFormat extends RichSequenceFormat.HeaderlessFormat
{

    /**
     * The over-ridden method.
     *
     * Essentially the selection of DNa or Protein parser has been changed to nucleotide or protein, and the selection
     * clause has been improved for clarity and function.
     *
     *
     *
     * @param stream
     *            the stream
     * @return the symbol tokenization
     * @throws IOException
    ...
    public SymbolTokenization guessSymbolTokenization(final BufferedInputStream stream) throws IOException
    {
        ...
        final String firstLine = buffReader.readLine();
        if (firstLine == null)
        {
            throw new EOFException("Out of data when identifying sequence type."); //$NON-NLS-1$
        }
        SymbolTokenization tok = null;

        if (firstLine.indexOf("DNA") > 0) //$NON-NLS-1$
        {
            tok = RichSequence.IOTools.getNucleotideParser();
        }
        else if (firstLine.indexOf("RNA") > 0) //$NON-NLS-1$
        {
            tok = RichSequence.IOTools.getNucleotideParser();
        }
        else
    ...
    protected static final Pattern lp = Pattern
            .compile("^(\\S+)\\s+\\d+\\s+(bp|aa)\\s{1,4}([dms]s-)?(\\S+)?\\s+(circular|linear)?\\s*(\\S+)?\\s*(\\S+)?$");
    ...
    protected static final Pattern refp = Pattern
            .compile("^(\\d+)\\s*(?:(\\((?:bases|residues)\\s+(\\d+\\s+to\\s+\\d+(\\s*;\\s*\\d+\\s+to\\s+\\d+)*)\\))|\\(sites\\))?");
    ...
    protected static final Pattern sectp = Pattern
            .compile("^(\\s{0,8}(\\S+)\\s{1,7}(.*)|\\s{21}(/\\S+?)=(.*)|\\s{21}(/\\S+))$");
    ...
    /**
     * {@inheritDoc} A file is in GenBank format if the name ends with gbk, contains the letters egb, or the first line
     * of the file starts with the word LOCUS
     */
    ...
    /**
     * {@inheritDoc} Returns an dna parser if the letters DNA or RNA appear in the first line of the file. Otherwise
     * returns a DNA tokenizer.
     */
    ...
    /**
     * {@inheritDoc} A stream is in GenBank format if the first line of the stream starts with the word LOCUS
     */
            ...
            Namespace ns) throws IllegalSymbolException, IOException, ParseException
    {
        ...
        try
        {
            do
            {
                ...
                if (sectionKey == null)
                {
                    String message = ParseException.newMessage(this.getClass(), accession, identifier,
                        "Section key was null", sectionToString(section));
                ...
                if (sectionKey.equals(LOCUS_TAG))
                {
                    ...
                    {
                        String message = ParseException.newMessage(this.getClass(), accession, identifier,
                            "Bad locus line", sectionToString(section));
                        throw new ParseException(message);
                    }
                }
                ...
                else if (sectionKey.equals(VERSION_TAG))
                {
                    ...
                    {
                        String message = ParseException.newMessage(this.getClass(), accession, identifier,
                            "Bad version line", sectionToString(section));
                        throw new ParseException(message);
                    }
                }
                ...
                else if (sectionKey.equals(REFERENCE_TAG) && !this.getElideReferences())
                {
                    ...
                    {
                        String message = ParseException.newMessage(this.getClass(), accession, identifier,
                            "Bad reference line", sectionToString(section));
                        throw new ParseException(message);
                    }
                    ...
                    try
                    {
                        ...
                        // assign the docref to the bioentry: null if no base ranges, Integers if 1 base range - the
                        // normal case, joined RichLocation if more than 1
                        RankedDocRef rdr = baseRangeList == null ? new SimpleRankedDocRef(dr, null, null, ref_rank)
                            : baseRangeList.size() == 1 ? new SimpleRankedDocRef(dr, new Integer(
                                ((RichLocation)baseRangeList.get(0)).getMin()), new Integer(
                                ((RichLocation)baseRangeList.get(0)).getMax()), ref_rank) : new SimpleRankedDocRef(dr,
                                new CompoundRichLocation(baseRangeList), ref_rank);
                ...
                else if (sectionKey.equals(FEATURE_TAG) && !this.getElideFeatures())
                {
                    ...
                    for (int i = 1; i < section.size(); i++)
                    {
                        ...
                        if (key.startsWith("/"))
                        {
                            if (!skippingBond)
                            {
                                ...
                                else
                                {
                                    ...
                                    rlistener.addFeatureProperty(RichObjectFactory.getDefaultOntology()
                                            .getOrCreateTerm(key), val);
                                }
                            }
                        }
                        else
                        {
                            ...
                            if (key.equalsIgnoreCase("bond") && val.startsWith("bond"))
                            {
                                // MD - Added convert the location string from bond(x,y) to x..y

                                val = val.replaceAll("bond", "").replaceAll("\\(", "").replaceAll("\\)", "")
                                        .replaceAll(",", "..");
                ...
                else if (sectionKey.equals(START_SEQUENCE_TAG) && !this.getElideSymbols())
                {
                    ...
                    try
                    {
                        SymbolList sl = new SimpleSymbolList(symParser, seq.toString().replaceAll("\\s+", "")
                                .replaceAll("[\\.|~]", "-"));
                    ...
                    {
                        String message = ParseException.newMessage(this.getClass(), accession, identifier,
                            "Bad sequence section", sectionToString(section));
        ...
        {
            String message = ParseException.newMessage(this.getClass(), accession, identifier, "Bad sequence section",
    ...
    private List readSection(BufferedReader br) throws ParseException
    {
            ...
            while (!done)
            {
                ...
                else
                {
                    ...
                    if (m.matches())
                    {
                        ...
                        currVal.append((m.group(2) == null ? (m.group(4) == null ? "" : m.group(5)) : m.group(3))
                                .trim());
                    }
                    else
                    {
        ...
        {
            String message = ParseException.newMessage(this.getClass(), accession, identifier, "Bad section",
                sectionToString(section));
            throw new ParseException(e, message);
        }
        return section;
    ...
    private final List buildBaseRanges(final String theBaseRangeList) throws ParseException
    {
        ...
        try
        {
            for (int r = 0; r < baseRange.length; r++)
            {
                ...
                if (rangeMatch.matches())
                {
                    ...
                    baseRangeList.add(new SimpleRichLocation(new SimplePosition(rangeStart), new SimplePosition(
                        rangeEnd), r));
                }
                else
                {
                    String message = ParseException.newMessage(this.getClass(), accession, identifier,
                        "Bad reference range found", theBaseRangeList);
                    throw new ParseException(message);
                }
            }
        ...
        {
            String message = ParseException.newMessage(this.getClass(), accession, identifier, "Bad base range",
                theBaseRangeList);
            throw new ParseException(e, message);
        }
    }
    ...
    public void writeSequence(Sequence seq, Namespace ns) throws IOException
    {
        ...
        // definition line
        StringTools.writeKeyValueLine(DEFINITION_TAG, rs.getDescription(), 12, this.getLineWidth(), this
                .getPrintStream());

        // accession line
        StringTools.writeKeyValueLine(ACCESSION_TAG, accessions.toString(), 12, this.getLineWidth(), this
                .getPrintStream());
        ...
        // keywords line
        keywords.append(".");
        StringTools.writeKeyValueLine(KEYWORDS_TAG, keywords.toString(), 12, this.getLineWidth() - 1, this
                .getPrintStream());
        ...
        if (tax != null)
        {
            StringTools.writeKeyValueLine(SOURCE_TAG, (isMitochondrial(rs) ? "mitochondrion " : "")
                + tax.getDisplayName(), 12, this.getLineWidth(), this.getPrintStream());
        ...
        for (Iterator r = rs.getRankedDocRefs().iterator(); r.hasNext();)
        {
            ...
            StringTools.writeKeyValueLine(REFERENCE_TAG, rdr.getRank()
                + (rdr.getLocation() == null || rdr.getLocation() == RichLocation.EMPTY_LOCATION ? ""
                    : (moltype == null ? "  (residues " : "  (bases ") + makeBaseRange(rdr) + ")"), 12, this
                    .getLineWidth(), this.getPrintStream());
            // Any authors that were in the input as CONSRTM tags will
            // be merged into the AUTHORS tag on output.
            StringTools.writeKeyValueLine("  " + AUTHORS_TAG, d.getAuthors(), 12, this.getLineWidth() - 1, this
                    .getPrintStream());
            StringTools.writeKeyValueLine("  " + TITLE_TAG, d.getTitle(), 12, this.getLineWidth(), this
                    .getPrintStream());
            StringTools.writeKeyValueLine("  " + JOURNAL_TAG, d.getLocation(), 12, this.getLineWidth(), this
                    .getPrintStream());
            CrossRef c = d.getCrossref();
            if (c != null)
            {
                StringTools.writeKeyValueLine(StringTools.leftPad(c.getDbname(), 9), c.getAccession(), 12, this
                        .getLineWidth(), this.getPrintStream());
            }
            StringTools.writeKeyValueLine("  " + REMARK_TAG, d.getRemark(), 12, this.getLineWidth(), this
                    .getPrintStream());
        }
        ...
        for (Iterator i = rs.getFeatureSet().iterator(); i.hasNext();)
        {
            ...
            StringTools.writeKeyValueLine("     " + f.getTypeTerm().getName(), GenbankLocationParser
                    .writeLocation((RichLocation)f.getLocation()), 21, this.getLineWidth() - 1, ",", this
                    .getPrintStream());
            for (Iterator j = f.getNoteSet().iterator(); j.hasNext();)
            {
                ...
                {
                    StringTools.writeKeyValueLine("", "/" + n.getTerm().getName(), 21, this.getLineWidth(), this
                            .getPrintStream());
                }
                else if (isNotQuoted(n))
                {// doesn't have the value enclosed in quotes
                    StringTools.writeKeyValueLine("", "/" + n.getTerm().getName() + "=" + n.getValue(), 21, this
                            .getLineWidth(), this.getPrintStream());
                }
                else if (n.getTerm().getName().equals("translation"))
                {
                    StringTools.writeKeyValueLine("", "/" + n.getTerm().getName() + "=\"" + n.getValue() + "\"", 21,
                        this.getLineWidth() - 1, this.getPrintStream());
                }
                else
                {
                    StringTools.writeKeyValueLine("", "/" + n.getTerm().getName() + "=\"" + n.getValue() + "\"", 21,
                        this.getLineWidth(), this.getPrintStream());
                }
            }
            ...
            if (f.getType().equals("source") && tax != null)
            {
                ...
                StringTools.writeKeyValueLine("", "/organism=\"" + displayName + "\"", 21, this.getLineWidth() - 1,
                ...
                for (Iterator j = f.getRankedCrossRefs().iterator(); j.hasNext();)
                {
                    ...
                    StringTools.writeKeyValueLine("", "/db_xref=\"" + cr.getDbname() + ":" + cr.getAccession() + "\"",
                        21, this.getLineWidth(), this.getPrintStream());
                }
                StringTools.writeKeyValueLine("", "/db_xref=\"taxon:" + tax.getNCBITaxID() + "\"", 21, this
                        .getLineWidth(), this.getPrintStream());
            }
            else
            {
    ...
    {
        return theReference.getLocation() == null ? theReference.getStart() + " to " + theReference.getEnd()
