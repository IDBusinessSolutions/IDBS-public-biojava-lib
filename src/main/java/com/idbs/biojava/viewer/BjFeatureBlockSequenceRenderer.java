...
import com.idbs.biojava.viewer.filter.*;
import com.idbs.biojava.viewer.filter.FilterUtils;
 ...
 * @author Thomas Down
 */
public class BjFeatureBlockSequenceRenderer extends AbstractChangeable
     ...
     */
    public BjFeatureBlockSequenceRenderer() {
        try {
     ...
     */
    public BjFeatureBlockSequenceRenderer(FeatureRenderer fRend) {
        try {
  ...
  public void paint(Graphics2D g, SequenceRenderContext src) {
    ...
    public SequenceViewerEvent processMouseEvent(SequenceRenderContext src,
        ...
        FeatureHolder hits =
            src.getFeatures().filter(new BjFeatureFilter.OverlapsLocation(new RangeLocation(sMin, sMax)),
                                     false);
